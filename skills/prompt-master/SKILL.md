...
**Google Flow** (powered by Veo 3.1 + Gemini Omni Flash)
Google Flow is Google's dedicated AI filmmaking platform. Veo 3.1 generates video with native audio. Gemini Omni Flash is the intelligence layer for editing, scene consistency, and understanding prompts. Two quality tiers: Fast and Quality. All output is SynthID watermarked.

**Prompt formula — use this order every time:**
...
**Prompt length:** 30–80 words. Sweet spot is 50–60 words. Long enough to be specific, short enough for the model to parse everything.

**Seven elements to layer in (use as many as relevant):**
1. Shot framing & camera motion
2. Visual style (realistic, cinematic, claymation, film noir, VHS, etc.)
3. Lighting (warm even light, harsh fluorescent, neon glow, golden hour, etc.)
4. Character description (specific: "woman in her twenties with wavy brown hair and light freckles")
5. Location (specific: "smoky jazz club at night," not "a bar")
6. Action (what the subject is doing and how)
7. Audio / dialogue (see audio section below)

**Camera vocabulary — use these terms directly in prompts:**
- *Angles:* low angle, eye level, high angle, bird's eye, worm's eye
- *Movements:* static/locked off, slow pan, tilt, dolly, tracking shot, crane shot, aerial view, handheld, POV shot
- *Shot sizes:* extreme close-up, close-up, medium shot, wide shot, two-shot
- *Lens/focus:* shallow depth of field, wide-angle lens, soft focus, macro lens, deep focus

**Audio prompting (Veo 3.1 generates audio natively — always describe it):**
- Dialogue: `A woman says, "We have to leave now."` — use quotation marks
- Sound effects: `SFX: thunder cracks in the distance`
- Ambient: `Ambient noise: the quiet hum of a starship bridge`
- Music/score: describe feel and instrumentation, not song titles
- Add `no dialogue` or `no music` explicitly if you want silence

**Negative prompts:** Describe what to exclude with descriptive language — *"a desolate landscape with no buildings or roads"* not *"no man-made structures."* Avoid leading with "no" — frame exclusions as scene descriptions.

**Advanced Flow features — prompt accordingly:**
- **Ingredients to Video:** User uploads up to 3 reference images (characters, objects, style references) before prompting. The prompt then directs what those ingredients do. Always instruct the user to upload ingredients first, then write the motion/action prompt around them.
- **Timestamp prompting** (multi-shot in one generation): Use `[00:00-00:02]` markers to direct a sequence shot by shot within a single prompt. Each timestamp block gets its own cinematography, action, and SFX line. Effective for 8-second clips with 2–4 distinct beats.
- **First and last frame:** User provides a start image and end image; the prompt describes the transition action and audio between them.
- **Add/remove object:** Uses Veo 2 (older model) — no audio generation. Note this limitation when writing prompts for this feature.

**Clip specs:** 4, 6, or 8 seconds per generation. 720p or 1080p. 16:9 or 9:16 aspect ratio. Specify in the prompt or setup note.

**What Flow is NOT:** It is not a single-prompt-to-full-video tool. Each clip is generated separately and assembled in the Scene Builder (timeline editor). Write prompts for individual shots, not full sequences — unless using timestamp prompting within one generation.
